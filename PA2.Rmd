...
```{r exploratoryanalysis, echo=TRUE, results='hide'}
...
We see it has **<span style="color:red">`r l`</span>** levels, which is far from the 48 official event types. We also see some of the values have been typed in upper case and others in lowercase, some values have added numbers and words, etc. In order to make this variable more useful we will: 
...
per2 <- per(mydata)
```
We can see we are currently at **<span style="color:red">`r per2` % </span>** . We will try to fix a few more typos.
```{r dataprocessing5, echo=TRUE}
...
per3 <- per(mydata)
```
We can see we are currently at **<span style="color:red">`r per3` % </span>** . This is much better! now lets subset the dataframe.
```{r dataprocessing6, echo=TRUE}
...
```
We have selected events that represent the top **<span style="color:red">`r per5` %</span>** fatalities. These Events are: 1) Tornado, 2) Excessive heat, 3) Flash flood, 4) Heat, 5) Lightning, 6) Thunderstorm wind, 7)Flood, 8) Rip current, 9) High wind, 10) Avalanche and 11) Winter storms.
...
```
We have selected events that represent the top **<span style="color:red">`r per6` %</span>** injuries. These Events are: 1) Tornado, 2) Thunderstorm wind, 3) Flood, 4) Excessive heat, 5) Lightning, 6) Heat, 7)Ice storm and 8) Flash flood.
...
plot3 <- mydata_total %>% ggplot(aes(x=EVTYPE,y=total)) +
        ...
        theme(axis.text.x = element_text(angle=20, vjust=0.6)) 
```
We have selected events that represent the top **<span style="color:red">`r per7` %</span>** fatalities & injuries. These Events are: 1) Tornado, 2) Thunderstorm wind, 3) Excessive heat, 4) Flood, 5) Lightning, 6) Heat, 7) Flash flood and 8) Ice storm.
...
```{r results5, echo=TRUE, fig.height=14, fig.width=7}
...
plot4 <- mydata_crop %>%   
        ...
        theme(axis.text.x = element_text(angle=20, vjust=0.6)) 
```
We have selected events that represent the top **<span style="color:red">`r per8` %</span>** of the total crop damage. These Events are: 1) Drought, 2) Flood, 3) Hurricane, 4) Ice storm, 5) Hail, 6) Flash flood, 7) Thunderstorm wind and 8) Frost/Freeze.
...
plot5 <- mydata_prop %>%   
        ...
        theme(axis.text.x = element_text(angle=20, vjust=0.6)) 
```
We have selected events that represent the top **<span style="color:red">`r per9` %</span>** of the total property damage. These Events are: 1) Flood, 2) Hurricane, 3) Tornado, 4) Flash flood, 5) Hail, 6) Thunderstorm wind, 7) Tropical storm and 8) Winter storm. 
...
plot6 <- mydata_total2 %>% ggplot(aes(x=EVTYPE,y=total)) +
        ...
        theme(axis.text.x = element_text(angle=20, vjust=0.6)) 
```
We have selected events that represent the top **<span style="color:red">`r per10` %</span>** of the total crop and property damage. These Events are: 1) Flood, 2) Hurricane, 3) Tornado, 4) Hail, 5) Flash flood, 6) Drought, 7) Thunderstorm wind and 8) Ice storm.  
...
```{r results9, echo=TRUE, fig.height=14, fig.width=7}
# by using the grid.arrange function we combine the 3 plots we created previously into a single figure
